---
name: irene-reproducing
description: Use when the user asks to make a result on Irene (CEA TGCC) reproducible, shareable, or turned into a notebook/script — or proactively suggest it after a chunk of exploratory job/file work that produced something worth preserving. Builds a Jupyter notebook that replays the real workflow via hpc_agent_core.client, not a hand-copied SSH script.
user-invocable: true
---

# Turning a Irene (CEA TGCC) session into a reproducible notebook
...
## Every call passes `facility="irene"` explicitly

```python
job = hpc.submit_job(facility="irene", spec={...})   # whatever the real spec was
status = hpc.wait_for_job(job["job_id"], facility="irene")
result = hpc.fs_download(facility="irene", remote_path="...", local_path="./downloads/result.json")
...
Record the first real Irene workflow with `mode="record"` only when TGCC is
reachable, then replay the completed cache offline. Preserve the chosen Bridge
partition, project, filesystem declaration, tasks/cores, modules, rendered
`#MSUB` script, job state, accounting, and output. State that the unified port
was built without live access until the recording proves the entire workflow.
...
5. **If a partition/GPU choice isn't given or obvious, check
   `get_resources(facility="irene")`** and pick from that — don't guess.
