---
name: fugaku-remote-command
description: Use when the user wants to run one or more shell commands on Fugaku's login node, or when another workflow needs run_command_on_cluster for a live check not covered by a safer typed tool. Requires an exact command preview and explicit user permission before execution.
user-invocable: true
---

# Running login-node commands on Fugaku

Every call in this skill uses `facility="fugaku"`.
