---
name: tsubame-reproducing
description: Use when the user asks to make a result on TSUBAME4.0 (Science Tokyo) reproducible, shareable, or turned into a notebook/script — or proactively suggest it after a chunk of exploratory job/file work that produced something worth preserving. Builds a Jupyter notebook that replays the real workflow via hpc_agent_core.client, not a hand-copied SSH script.
user-invocable: true
---

# Turning a TSUBAME4.0 (Science Tokyo) session into a reproducible notebook
...
## Every call passes `facility="tsubame"` explicitly

```python
job = hpc.submit_job(facility="tsubame", spec={...})   # whatever the real spec was
status = hpc.wait_for_job(job["job_id"], facility="tsubame")
result = hpc.fs_download(facility="tsubame", remote_path="...", local_path="./downloads/result.json")
...
Record the first real TSUBAME workflow with `mode="record"` only when the
facility is reachable, then replay the completed cache offline. Include the
chosen resource type and count, group or explicit trial choice, priority,
modules, launcher, rendered Grid Engine script, job status, and output. State
that the unified-hub port was built without live access until a fresh recording
proves otherwise.
...
5. **If a partition/GPU choice isn't given or obvious, check
   `get_resources(facility="tsubame")`** and pick from that — don't guess.
