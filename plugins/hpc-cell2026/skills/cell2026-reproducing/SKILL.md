---
name: cell2026-reproducing
description: Use when the user asks to make a result on cell2026 (Shinobu Lab) reproducible, shareable, or turned into a notebook/script — or proactively suggest it after a chunk of exploratory job/file work that produced something worth preserving. Builds a Jupyter notebook that replays the real workflow via hpc_agent_core.client, not a hand-copied SSH script.
user-invocable: true
---

# Turning a cell2026 (Shinobu Lab) session into a reproducible notebook
...
## Every call passes `facility="cell2026"` explicitly

```python
job = hpc.submit_job(facility="cell2026", spec={...})   # whatever the real spec was
status = hpc.wait_for_job(job["job_id"], facility="cell2026")
result = hpc.fs_download(facility="cell2026", remote_path="...", local_path="./downloads/result.json")
...
Record one workflow per scheduler with `mode="record"` only while cell2026 is
reachable, then replay the completed cache offline. Preserve the queue/host
selector, scheduler, resources, parallel environment, container choice,
rendered script, registry-routed status, and output. For Slurm, download or
save output before its scheduler record expires. State that the original code
was live-tested in 2026 but this unified port needs a fresh recording.
...
5. **If a partition/GPU choice isn't given or obvious, check
   `get_resources(facility="cell2026")`** and pick from that — don't guess.
