---
name: irene-remote-command
description: Use when the user wants to run one or more shell commands on Irene (CEA TGCC)'s login node, or when another workflow needs run_command_on_cluster for a live check not covered by a safer typed tool. Requires an exact command preview and explicit user permission before execution.
user-invocable: true
---

# Running login-node commands on Irene (CEA TGCC)

Every call in this skill uses `facility="irene"`.
...
> I propose three short, read-only checks on Irene (CEA TGCC):
>
