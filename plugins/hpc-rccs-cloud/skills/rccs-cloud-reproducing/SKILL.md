...
description: Use when the user asks to make a result on R-CCS Cloud reproducible, shareable, or turned into a notebook/script — or proactively suggest it after a chunk of exploratory job/file work that produced something worth preserving. Builds a Jupyter notebook that replays the real workflow via hpc_agent_core.client, not a hand-copied SSH script.
user-invocable: true
---

# Turning a R-CCS Cloud session into a reproducible notebook
...
## Every call passes `facility="rccs-cloud"` explicitly

```python
job = hpc.submit_job(facility="rccs-cloud", spec={...})   # whatever the real spec was
status = hpc.wait_for_job(job["job_id"], facility="rccs-cloud")
result = hpc.fs_download(facility="rccs-cloud", remote_path="...", local_path="./downloads/result.json")
...
5. **If a partition/GPU choice isn't given or obvious, check
   `get_resources(facility="rccs-cloud")`** and pick from that — don't guess.
